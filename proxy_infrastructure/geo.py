...
geo.py
======
مدير الجغرافيا والربط الذكي للأقاليم (Country Geo Mapper & Fallback Router).
يقوم باستخراج كود الدولة من بادئة رقم الهاتف وتحديد التراجع الجغرافي الإقليمي.
"""
...
# خريطة بادئات الهاتف الدولية إلى كود الدولة (ISO 3166-1 alpha-2)
# تم نقلها لضمان البناء المستقل
...
# خريطة التكتلات الإقليمية (Regional Clusters)
# تُستخدم للـ Fallback الجغرافي عندما لا تتوفر بروكسيات لنفس دولة الرقم.
# قمنا بتقسيم العالم إلى 5 أقاليم تطابق توزيع مراكز بيانات تيليجرام (DCs) لتقليل زمن الاستجابة.
REGIONAL_CLUSTERS = {
    # إقليم أوروبا والشرق الأوسط (DC2/DC4)
    "EUROPE_ME": [
        "DE", "FR", "GB", "NL", "BE", "ES", "IT", "RO", "CH", "AT", "DK", "SE", "NO", "PL", "GR", "HU", "CZ", "SK", "IE", "FI", "UA", "TR",
        "SA", "AE", "KW", "QA", "OM", "BH", "JO", "LB", "EG", "MA", "DZ", "TN", "LY", "YE", "IQ", "SY", "PS"
    ],
    # إقليم آسيا والمحيط الهادئ (DC5)
    "ASIA_PACIFIC": [
        "SG", "MY", "TH", "ID", "PH", "VN", "JP", "KR", "CN", "TW", "HK", "IN", "PK", "BD", "LK", "AU", "NZ", "KH", "LA", "MM"
    ],
    # إقليم أمريكا الشمالية (DC1)
    "NORTH_AMERICA": [
        "US", "CA", "MX", "PR", "DO", "JM"
    ],
    # إقليم أمريكا الجنوبية (DC1/DC3)
    "SOUTH_AMERICA": [
        "BR", "AR", "CO", "CL", "PE", "VE", "EC", "UY", "PY", "BO"
    ],
    # إقليم أفريقيا جنوب الصحراء (DC2/DC4)
    "SUB_SAHARAN_AFRICA": [
        "ZA", "NG", "KE", "GH", "SN", "TZ", "UG", "ET", "AO", "MZ", "CM", "CI"
    ]
}
...
def get_fallback_countries(country_code: str) -> list:
    """
    إرجاع قائمة الدول البديلة المرتبطة بنفس الإقليم الجغرافي للدولة المدخلة.
    تُرتب الدول لتسهيل البحث عن بروكسي إقليمي بديل.
    """
    country_code = country_code.upper()
    for region, countries in REGIONAL_CLUSTERS.items():
        if country_code in countries:
            # نرجع قائمة الدول في الإقليم مع استبعاد الدولة الحالية
            return [c for c in countries if c != country_code]
    return []
